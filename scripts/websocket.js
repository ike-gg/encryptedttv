...
const replaceWebsocket = (key) => {
  window.WebSocket.prototype.oldSend = window.WebSocket.prototype.send;

  window.WebSocket.prototype.send = function (data) {
    const isSendMessageRequest =
      typeof data === "string" && data.includes("PRIVMSG");

    if (!isSendMessageRequest) {
      window.WebSocket.prototype.oldSend.apply(this, [data]);
      return;
    }

    const messageContent = data.split(":").splice(1).join("");

    const encryptedContent = `[ETV] ${CryptoJS.AES.encrypt(
      messageContent,
      key
    ).toString()}`;
    ...
    const modifiedData =
      typeof key === "string" && key
        ? data.replace(messageContent, encryptedContent)
        : data;

    window.WebSocket.prototype.oldSend.apply(this, [modifiedData]);
  };
};

const checkingForElemenet = setInterval(() => {
  const inputKeyElement = document.getElementById("etv-key");
  if (inputKeyElement) {
    key = inputKeyElement.value;
    replaceWebsocket(key);
    clearInterval(checkingForElemenet);
  }
}, 50);
